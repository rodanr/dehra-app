...
from user_management.schemas.user import UserSignUpSchema, UserLogInSchema, UserPasswordChangeSchema, UserEmailChange, \
...
class UserPasswordChange(Resource):
    ...
    def post(cls):
        ...
        user_data_for_password_change = user_password_change_schema.load(user_password_change_json)
        user = UserModel.find_by_username(user_data_for_password_change['username'])
        if not user:
            return {'message': USER_NOT_FOUND}, 400  # Bad Request
        if user.password and check_password_hash(user.password, user_data_for_password_change['old_password']):
            user.password = generate_password_hash(user_data_for_password_change['new_password'])
            user.save_to_db()
...
class UserEmailChange(Resource):
    ...
    def post(cls):
        ...
        user_data_for_email_change = user_email_change_schema.load(user_email_change_json)
...
class UserMobileNumberChange(Resource):
    ...
    def post(cls):
        ...
        user_data_for_mobile_number_change = user_mobile_number_change_schema.load(user_mobile_number_change_json)
        user = UserModel.find_by_username(user_data_for_mobile_number_change['username'])
        if not user:
            return {'message': USER_NOT_FOUND}, 400  # Bad Request
